...
class BigComCatalogGetAllCategoriesOpConfigModel(BaseOpConfigModel):
...
class BigComCatalogGetAllCategoriesOpMetadataModel(BaseOpMetadataModel):
    current_page: int
    total_pages: int
    total_records: int
...
class BigComCatalogGetAllCategoriesOpAuditModel(BaseOpAuditModel):
    pass
...
class BigComCatalogGetAllCategoriesOp(BaseGeneratorOp):
    """.
    ...
    OP_TYPE = "generator.bigcom.catalog.get_all_categories"
    OP_CONFIG_MODEL = BigComCatalogGetAllCategoriesOpConfigModel
    OP_METADATA_MODEL = BigComCatalogGetAllCategoriesOpMetadataModel
    OP_AUDIT_MODEL = BigComCatalogGetAllCategoriesOpAuditModel
    ...
    def get_page(self, page: int) -> Tuple[int, Dict[str, Any], Dict[str, Any]]:
        ...
        url = (
            f"{self.http_conn.base_url}/stores/{self.store_hash}/v3/catalog/categories"
        )
        logger.info(
            f"BigComCatalogGetAllCategoriesOp.get_page: Fetching {page=} with {url=}"
        )
    ...
    def run(
        ...
        """."""
        logger.info(f"BigComCatalogGetAllCategoriesOp.run: Running")

        # RENDERS AND UPDATES THE TEMPLATED FIELDS INPLACE
        self.render_fields(
            time_step=time_step,
            msg=msg,
            log_prefix="BigComCatalogGetAllCategoriesOp.run:",
        )

        final_records = []
        page, total_records, total_pages = 1, 0, None
        while True:
            logger.info(f"BigComCatalogGetAllCategoriesOp.run: Fetching {page=}.")
            ...
            logger.info(
                f"BigComCatalogGetAllCategoriesOp.run: Done fetching {page=}. "
            ...
            if self.iterate_over_pages:
                yield OpMsg(
                    data=final_records,
                    metadata=BigComCatalogGetAllCategoriesOpMetadataModel(
                        current_page=page,
                        total_pages=total_pages,
                        total_records=total_records,
                    ),
                    audit=BigComCatalogGetAllCategoriesOpAuditModel(),
                )
                final_records = []
            ...
            if page >= total_pages:
                logger.info(
                    f"BigComCatalogGetAllCategoriesOp.run: Fetched {total_pages=}. "
                    f"Yielding records."
                )
                break

            logger.info(
                f"BigComCatalogGetAllCategoriesOp.run: Sleeping for "
        ...
        if self.iterate_over_pages:
            logger.info(
                f"BigComCatalogGetAllCategoriesOp.run: {self.iterate_over_pages=}. "
