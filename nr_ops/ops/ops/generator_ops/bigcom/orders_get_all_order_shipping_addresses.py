...
class BigComOrdersGetAllOrderShippingAddressesOpConfigModel(BaseOpConfigModel):
...
class BigComOrdersGetAllOrderShippingAddressesOpMetadataModel(BaseOpMetadataModel):
    current_page: int
    total_pages: int
    total_records: int
...
class BigComOrdersGetAllOrderShippingAddressesOpAuditModel(BaseOpAuditModel):
    pass
...
class BigComOrdersGetAllOrderShippingAddressesOp(BaseGeneratorOp):
    """.
    ...
    OP_TYPE = "generator.bigcom.orders.get_all_order_shipping_addresses"
    OP_CONFIG_MODEL = BigComOrdersGetAllOrderShippingAddressesOpConfigModel
    OP_METADATA_MODEL = BigComOrdersGetAllOrderShippingAddressesOpMetadataModel
    OP_AUDIT_MODEL = BigComOrdersGetAllOrderShippingAddressesOpAuditModel
    ...
    def get_page(self, page: int) -> Tuple[int, str, Dict[str, Any]]:
        ...
        url = f"{self.http_conn.base_url}/stores/{self.store_hash}/v2/orders/{self.order_id}/shipping_addresses"
        logger.info(
            f"BigComOrdersGetAllOrderShippingAddressesOp.get_page: Fetching {page=} with {url=}"
        )
    ...
    def run(
        ...
        """."""
        logger.info(f"BigComOrdersGetAllOrderShippingAddressesOp.run: Running")

        # RENDERS AND UPDATES THE TEMPLATED FIELDS INPLACE
        self.render_fields(
            time_step=time_step,
            msg=msg,
            log_prefix="BigComOrdersGetAllOrderShippingAddressesOp.run:",
        )
        ...
        while True:
            logger.info(
                f"BigComOrdersGetAllOrderShippingAddressesOp.run: Fetching {page=}."
            )

            status_code, output_text, etl_metadata_json = self.get_page(page=page)
            if status_code == 204:
                logger.info(
                    f"BigComOrdersGetAllOrderShippingAddressesOp.run: Received 204. "
            ...
            logger.info(
                f"BigComOrdersGetAllOrderShippingAddressesOp.run: Done fetching {page=}. "
            ...
            logger.info(
                f"BigComOrdersGetAllOrderShippingAddressesOp.run: Sleeping for "
        ...
        yield OpMsg(
            data=final_records,
            metadata=BigComOrdersGetAllOrderShippingAddressesOpMetadataModel(
                current_page=page,
                total_pages=total_pages,
                total_records=total_records,
            ),
            audit=BigComOrdersGetAllOrderShippingAddressesOpAuditModel(),
        )
