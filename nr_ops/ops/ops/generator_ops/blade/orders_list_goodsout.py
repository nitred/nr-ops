...
class FilterConfigModel(BaseModel):
    filter_by: Literal["date_placed", "despatch_date"]
    filter_op: Literal["LESS_THAN", "ON_DATE", "MORE_THAN"]
    filter_date: StrictStr


class BladeOrdersListGoodsoutOpConfigModel(BaseOpConfigModel):
...
class BladeOrdersListGoodsoutOpMetadataModel(BaseOpMetadataModel):
    page: int
    total_pages: int
    n_records: int
...
class BladeOrdersListGoodsoutOpAuditModel(BaseOpAuditModel):
    pass
...
class BladeOrdersListGoodsoutOp(BaseGeneratorOp):
    """.

    NOTES:
    - The `page` field in the metadata is 1-indexed.
    - For some reason requests with a high page number (>20???) returns 500s
        - Therefore the appropriate strategy is to list_goodsout with the `date_placed`
          or `despatch_date` filter such that the number of pages is low.
    """

    OP_TYPE = "generator.blade.orders.list_goodsout"
    OP_CONFIG_MODEL = BladeOrdersListGoodsoutOpConfigModel
    OP_METADATA_MODEL = BladeOrdersListGoodsoutOpMetadataModel
    OP_AUDIT_MODEL = BladeOrdersListGoodsoutOpAuditModel
    ...
    def get_page(self, page: int) -> Tuple[Dict[str, Any], Dict[str, Any]]:
        ...
        url = (
            f"{self.http_conn.base_url}/v1/orders/goodsouts"
    ...
    def run(
        ...
        """."""
        logger.info(f"BladeOrdersListGoodsoutOp.run: Running")

        # RENDERS AND UPDATES THE TEMPLATED FIELDS INPLACE
        self.render_fields(
            time_step=time_step, msg=msg, log_prefix="BladeOrdersListGoodsoutOp.run:"
        )
        ...
        while (total_pages is None) or (page <= total_pages):
            ...
            if total_pages:
                logger.info(
                    f"BladeOrdersListGoodsoutOp.run: Sleeping for "
            ...
            logger.info(
                f"BladeOrdersListGoodsoutOp.run: Fetching {page=} of {total_pages=}."
            )
            ...
            logger.info(
                f"BladeOrdersListGoodsoutOp.run: Done fetching {page=} of {total_pages=}. "
            ...
            if self.remove_pii:
                logger.info(
                    f"BladeOrdersListGoodsoutOp.run: Redacting PII from the data."
                )
                # Redact PII from the data in place.
                for order in output_json["data"]:
                    for key in [
                        "first_name",
                        "last_name",
                        "email",
                ...
                logger.info(
                    f"BladeOrdersListGoodsoutOp.run: Done redacting PII from the data."
                )
            ...
            logger.info(
                f"BladeOrdersListGoodsoutOp.run: Yielding results for page "
                f"{page}/{total_pages}."
            )

            yield OpMsg(
                data=output_json,
                metadata=BladeOrdersListGoodsoutOpMetadataModel(
                    page=page,
                ...
                audit=BladeOrdersListGoodsoutOpAuditModel(),
            )
