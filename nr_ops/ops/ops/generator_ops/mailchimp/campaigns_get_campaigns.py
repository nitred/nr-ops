...
class MailchimpCampaignsGetCampaignsOpConfigModel(BaseOpConfigModel):
...
class MailchimpCampaignsGetCampaignsOpMetadataModel(BaseOpMetadataModel):
    current_page: int
    total_pages: int
    total_records: int
...
class MailchimpCampaignsGetCampaignsOpAuditModel(BaseOpAuditModel):
    pass
...
class MailchimpCampaignsGetCampaignsOp(BaseGeneratorOp):
    """."""

    OP_TYPE = "generator.mailchimp.campaigns.get_campaigns"
    OP_CONFIG_MODEL = MailchimpCampaignsGetCampaignsOpConfigModel
    OP_METADATA_MODEL = MailchimpCampaignsGetCampaignsOpMetadataModel
    OP_AUDIT_MODEL = MailchimpCampaignsGetCampaignsOpAuditModel
    ...
    def get_page(self, page: int) -> Tuple[int, Dict[str, Any], Dict[str, Any]]:
        ...
        if self.min_date_created:
            params["since_create_time"] = self.min_date_created
        ...
        logger.info(
            f"MailchimpCampaignsGetCampaignsOp.get_page: Fetching {page=} with {url=}"
        )
    ...
    def run(
        ...
        """."""
        logger.info(f"MailchimpCampaignsGetCampaignsOp.run: Running")

        # RENDERS AND UPDATES THE TEMPLATED FIELDS INPLACE
        self.render_fields(
            time_step=time_step,
            msg=msg,
            log_prefix="MailchimpCampaignsGetCampaignsOp.run:",
        )

        final_records = []
        page, total_records, total_pages = 1, 0, None
        while True:
            logger.info(f"MailchimpCampaignsGetCampaignsOp.run: Fetching {page=}.")

            status_code, output_json, etl_metadata_json = self.get_page(page=page)
            if len(output_json["campaigns"]) == 0:
                logger.info(
                    f"MailchimpCampaignsGetCampaignsOp.run: Received 0 campaigns. "
            ...
            n_records = len(output_json["campaigns"])
            total_records += n_records

            logger.info(
                f"MailchimpCampaignsGetCampaignsOp.run: Done fetching {page=}. "
            ...
            if self.iterate_over_pages:
                yield OpMsg(
                    data=final_records,
                    metadata=MailchimpCampaignsGetCampaignsOpMetadataModel(
                        current_page=page,
                        total_pages=-1,
                        total_records=total_records,
                    ),
                    audit=MailchimpCampaignsGetCampaignsOpAuditModel(),
                )
                final_records = []

            logger.info(
                f"MailchimpCampaignsGetCampaignsOp.run: Sleeping for "
        ...
        if self.iterate_over_pages:
            logger.info(
                f"MailchimpCampaignsGetCampaignsOp.run: {self.iterate_over_pages=}. "
