...
class MailchimpCampaignsGetSentToOpConfigModel(BaseOpConfigModel):
...
class MailchimpCampaignsGetSentToOpMetadataModel(BaseOpMetadataModel):
    current_page: int
    total_pages: int
    total_records: int
...
class MailchimpCampaignsGetSentToOpAuditModel(BaseOpAuditModel):
    pass
...
class MailchimpCampaignsGetSentToOp(BaseGeneratorOp):
    """."""

    OP_TYPE = "generator.mailchimp.campaigns.get_sent_to"
    OP_CONFIG_MODEL = MailchimpCampaignsGetSentToOpConfigModel
    OP_METADATA_MODEL = MailchimpCampaignsGetSentToOpMetadataModel
    OP_AUDIT_MODEL = MailchimpCampaignsGetSentToOpAuditModel
    ...
    def get_page(self, page: int) -> Tuple[int, Dict[str, Any], Dict[str, Any]]:
        ...
        url = f"{self.http_conn.base_url}/reports/{self.campaign_id}/sent-to"
        logger.info(
            f"MailchimpCampaignsGetSentToOp.get_page: Fetching {page=} with {url=}"
        )
    ...
    def run(
        ...
        """."""
        logger.info(f"MailchimpCampaignsGetSentToOp.run: Running")

        # RENDERS AND UPDATES THE TEMPLATED FIELDS INPLACE
        self.render_fields(
            time_step=time_step,
            msg=msg,
            log_prefix="MailchimpCampaignsGetSentToOp.run:",
        )

        final_records = []
        page, total_records, total_pages = 1, 0, None
        while True:
            logger.info(f"MailchimpCampaignsGetSentToOp.run: Fetching {page=}.")

            status_code, output_json, etl_metadata_json = self.get_page(page=page)
            if len(output_json["sent_to"]) == 0:
                logger.info(
                    f"MailchimpCampaignsGetSentToOp.run: Received 0 sent_to records. "
            ...
            n_records = len(output_json["sent_to"])
            total_records += n_records

            logger.info(
                f"MailchimpCampaignsGetSentToOp.run: Done fetching {page=}. "
            ...
            if self.remove_pii:
                logger.info(
                    f"MailchimpCampaignsGetSentToOp.run: "
                    f"Redacting PII from the data."
                )
                # Redact PII from the data in place.
                for record in output_json["sent_to"]:
                    for key in [
                        "email_address",
                        "merge_fields",
                        "full_name",
            ...
            if self.iterate_over_pages:
                yield OpMsg(
                    data=final_records,
                    metadata=MailchimpCampaignsGetSentToOpMetadataModel(
                        current_page=page,
                        total_pages=-1,
                        total_records=total_records,
                    ),
                    audit=MailchimpCampaignsGetSentToOpAuditModel(),
                )
                final_records = []

            logger.info(
                f"MailchimpCampaignsGetSentToOp.run: Sleeping for "
        ...
        if self.iterate_over_pages:
            logger.info(
                f"MailchimpCampaignsGetSentToOp.run: {self.iterate_over_pages=}. "
